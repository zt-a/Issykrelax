...
class CheckInUseCase:
    ...
    async def execute(self, verification_code: str, owner_id: UUID) -> BookingResponse:
        booking = await self._booking_repo.get_by_verification_code(verification_code)
        if not booking:
        ...
        property = await self._property_repo.get_by_id(booking.property_id)
        if not property or property.owner_id != owner_id:
            raise ValueError("This booking does not belong to your property")
        ...
        if both_confirmed:
            ...
            provider_wallet = await self._wallet_repo.get_by_user_id(owner_id)
            if not provider_wallet:
                provider_wallet = await self._wallet_repo.create_for_user(owner_id)
