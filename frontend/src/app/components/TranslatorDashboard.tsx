...
import { Languages, Edit, Wallet, BarChart3, CheckCircle, Clock, DollarSign } from "lucide-react";
import { toast } from "sonner";
import { Helmet } from "react-helmet-async";
import { getMyTranslatorProfile, createTranslatorProfile, updateTranslatorProfile } from "../services/translator";
...
export function TranslatorDashboard({ onNavigate }: TranslatorDashboardProps) {
  ...
  const load = async () => {
    try {
      const [p, w] = await Promise.all([
        getMyTranslatorProfile().catch(() => null),
        getMyWallet().catch(() => null),
      ]);
      setProfile(p);
      setWalletData(w);
      if (p) { setBio(p.bio || ""); setLanguages(p.languages || ""); }
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      if (profile) { await updateTranslatorProfile({ bio, languages }); }
      else { await createTranslatorProfile({ bio, languages }); }
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--background)" }}>
      <Helmet><title>Панель переводчика | IssykRelax</title><meta name="robots" content="noindex, nofollow" /></Helmet>

      <div className="py-10 px-4" style={{ background: "linear-gradient(135deg, #7c3aed 0%, #a78bfa 100%)" }}>
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-1">
            <Languages size={28} className="text-white/80" />
            <h1 className="text-3xl font-bold text-white" style={{ fontFamily: "var(--font-display)" }}>Панель переводчика</h1>
          </div>
          ...
          <>
            ...
            {profile && (
              <div className="rounded-2xl border p-6 mb-6" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <span className="text-xs font-semibold" style={{ color: "var(--text-secondary)" }}>ЯЗЫКИ</span>
                    <p className="text-sm mt-1" style={{ color: "var(--text-primary)" }}>{profile.languages || "—"}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {!profile && (
              <div className="rounded-2xl border p-10 text-center" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
                <Languages size={48} className="mx-auto mb-3" style={{ color: "var(--text-secondary)" }} />
          ...
          <div className="w-full max-w-md rounded-2xl border p-6 custom-modal" style={{ borderColor: "var(--border)", background: "var(--card)" }}>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input value={languages} onChange={(e) => setLanguages(e.target.value)} className="w-full border rounded-xl px-3 py-2.5 text-sm outline-none" style={{ borderColor: "var(--border)", background: "var(--background)", color: "var(--text-primary)" }} />
              </div>
